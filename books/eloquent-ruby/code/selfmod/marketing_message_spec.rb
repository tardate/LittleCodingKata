...
module WithDefineMethod
  describe MarketingMessage do
    it "has the replace_ methods" do
      methods = MarketingMessage.instance_methods
      expect(methods).to include(:replace_title)
      expect(methods).to include(:replace_firstname)
      expect(methods).to include(:replace_lastname)
    end
