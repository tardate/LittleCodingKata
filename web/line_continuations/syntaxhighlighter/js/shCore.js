...
var dp = {
  ...
  // Function to substitute the leading 'prefixChar' characters in the string with the 'replacement'
  // added for wraplines support
  function replacePrefix(line,prefixChar,replacement){
    if(line.indexOf(prefixChar) == -1)
    	return line;
    var pos=0;
    var newPrefix='';
  	while(line.charAt(pos) == prefixChar)
  	{
  		pos++;
  		newPrefix=newPrefix.concat(replacement);
  	}
    return newPrefix + line.substr(pos);  	
  }
...
dp.sh.Highlighter = function()
{
	...
	this.wrapLines = false; // added for wraplines support
}
...
//	str = str.replace(/&/g, '&amp;');
 	str = str.replace(/ /g, '&nbsp;'); // must initially map all whitespace since IE won't preserve it
	str = str.replace(/</g, '&lt;');
//	str = str.replace(/&lt;/g, '<');
//	str = str.replace(/>/g, '&gt;');
	str = str.replace(/\n/gm, '&nbsp;<br>');
...
dp.sh.Highlighter.prototype.SwitchToList = function()
{
  ...
  // thanks to Lachlan Donald from SitePoint.com for this <br/> tag fix.
	...
	for(var i = 0, lineIndex = this.firstLine; i < lines.length - 1; i++, lineIndex++)
	{
		...
		if (this.wrapLines) { 
  		// map all &nbsp; back to ' ' except leading characters; added for wraplines support
		  var line = lines[i].replace(/&nbsp;/g, ' ');
		  lines[i] = replacePrefix(line, ' ', '&nbsp;')
		} 
		span.innerHTML = lines[i] + '&nbsp;';
...
dp.sh.HighlightAll = function(name, showGutter /* optional */, showControls /* optional */, collapseAll /* optional */, firstLine /* optional */, showColumns /* optional */, wrapLines /* optional */) // modified for wraplines support
{
	function FindValue()
	{
		var a = arguments;
	...
	for(var i = 0; i < elements.length; i++)
	{
		...
		highlighter.wrapLines = (wrapLines == null) ? IsOptionSet('wraplines', options) : wrapLines; // added for wraplines support
