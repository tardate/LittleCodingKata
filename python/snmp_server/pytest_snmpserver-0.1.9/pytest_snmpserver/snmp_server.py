...
def craft_response(version, community, request_id, error_status, error_index, oid_items):
    ...
    response = write_tv(
        ...
        write_tv(
            ...
            write_tv(ASN1_INTEGER, _write_int(request_id, strip_leading_zeros=False)) +
...
class SNMPServer:
    ...
    def process_request(self):
        while self._is_running:
            try:
                ...
                if e.errno == 9:  # bad file descriptor -> broken socket
                    logger.debug('Socket broke while waiting for data')
                    continue
                raise
    ...
    def expect_request(self, request_id, reply_with, populate_parent=True):
        if isinstance(reply_with, str):
            ...
            logging.debug('wtf!!', reply_with)
            reply = reply_with

        logging.debug(request_id, reply)
